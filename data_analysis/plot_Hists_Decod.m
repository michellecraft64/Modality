...
%WCM rank sum
pW_NDbic=ranksum(DecA_ND,DecA_Bic);
pW_NDmus=ranksum(DecA_ND,DecA_Mus);
%One-way ANOVA
g_NDbic=[ones(length(DecA_ND),1);2*ones(length(DecA_Bic),1)];
g_NDmus=[ones(length(DecA_ND),1);2*ones(length(DecA_Mus),1)];
pA_NDbic = anova1([DecA_ND; DecA_Bic],g_NDbic,'off');
pA_NDmus = anova1([DecA_ND; DecA_Mus],g_NDmus,'off');
