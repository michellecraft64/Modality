function [A_M,A_P,synE,synI,w_MP,Atrn_M,Atrn_P,w_MPt]=...
            WCnet_EIcellsF(CpldWx,TimeVars,TimeConstants,xMin,InputCurrent,crr)
...
% w_MP' = 1/tw*[ -w_MP + F(A_m) ] where F(x)=2./(1+exp(-(x-0)/1))-1
    ...
    Fac=@(x)(2./(1+exp(-(x-0)./1))-1); %facilitatation instead of Depress
    ...
    tme=(0:dt:lenSpon+lenEvok)'; %time after transients from 0-tEnd
    ...
    for i = 2:lenTrnS
        ...
        rhs1_Pt = dt/tau_P*(-Atrn_P(i-1) + (w_I*Itrn_P + (wmp0+0*w_MPt(i-1)).*synEtrn(i-1) > xP_min).*(w_I*Itrn_P + (wmp0+0*w_MPt(i-1)).*synEtrn(i-1)));
        asye1 = dt*(-asye/trE+Atrn_M(i-1));
        synE1 = dt/tdE*(-synEtrn(i-1)+asye);
        asyi1 = dt*(-asyi/trI+gnIsy*Atrn_P(i-1));
        synI1 = dt/tdI*(-synItrn(i-1)+asyi);
        w_MPt1 = dt/tw*crr*(Fac(Atrn_M(i-1))-w_MPt(i-1)); %Facilit

        %k2
        rhs2_Mt = dt/tau_M*(-(Atrn_M(i-1)+0.5*rhs1_Mt) + (w_I*Itrn_M - w_P.*(synItrn(i-1)+0.5*synI1) > xM_min).*(w_I*Itrn_M - w_P.*(synItrn(i-1)+0.5*synI1)));
        rhs2_Pt = dt/tau_P*(-(Atrn_P(i-1)+0.5*rhs1_Pt) + (w_I*Itrn_P + (wmp0+0*w_MPt(i-1)+0.5*w_MPt1).*(synEtrn(i-1)+0.5*synE1) > xP_min).*(w_I*Itrn_P + (wmp0+0*w_MPt(i-1)+0.5*w_MPt1).*(synEtrn(i-1)+0.5*synE1)));
        ...
        w_MPt2 = dt/tw*crr*(Fac(Atrn_M(i-1)+.5*rhs1_Pt)-(w_MPt(i-1)+.5*w_MPt1));
            
        %k3
        rhs3_Mt = dt/tau_M*(-(Atrn_M(i-1)+0.5*rhs2_Mt) + (w_I*Itrn_M - w_P.*(synItrn(i-1)+0.5*synI2) > xM_min).*(w_I*Itrn_M - w_P.*(synItrn(i-1)+0.5*synI2)));
        rhs3_Pt = dt/tau_P*(-(Atrn_P(i-1)+0.5*rhs2_Pt) + (w_I*Itrn_P + (wmp0+0*w_MPt(i-1)+0.5*w_MPt2).*(synEtrn(i-1)+0.5*synE2) > xP_min).*(w_I*Itrn_P + (wmp0+0*w_MPt(i-1)+0.5*w_MPt2).*(synEtrn(i-1)+0.5*synE2)));
        ...
        w_MPt3 = dt/tw*crr*(Fac(Atrn_M(i-1)+.5*rhs2_Pt)-(w_MPt(i-1)+.5*w_MPt2));
        
        %k4
        rhs4_Mt = dt/tau_M*(-(Atrn_M(i-1)+rhs3_Mt) + (w_I*Itrn_M - w_P.*(synItrn(i-1)+synI3) > xM_min).*(w_I*Itrn_M - w_P.*(synItrn(i-1)+synI3)));
        rhs4_Pt = dt/tau_P*(-(Atrn_P(i-1)+rhs3_Pt) + (w_I*Itrn_P + (wmp0+0*w_MPt(i-1)+w_MPt3).*(synEtrn(i-1)+synE3) > xP_min).*(w_I*Itrn_P + (wmp0+0*w_MPt(i-1)+w_MPt3).*(synEtrn(i-1)+synE3)));
        ...
        w_MPt4 = dt/tw*crr*(Fac(Atrn_M(i-1)+rhs3_Pt)-(w_MPt(i-1)+w_MPt3));
    ...
    for j=2:lenTime
        ...
        w_MP1 = dt/tw*crr*(Fac(A_M(j-1))-w_MP(j-1)); %%Facilit, presynaptic = MC postsynaptic = PGC
        ...
        w_MP2 = dt/tw*crr*(Fac(A_M(j-1)+.5*rhs1_Pt)-(w_MP(j-1)+.5*w_MP1));
        ...
        w_MP3 = dt/tw*crr*(Fac(A_M(j-1)+.5*rhs2_Pt)-(w_MP(j-1)+.5*w_MP2));
        ...
        w_MP4 = dt/tw*crr*(Fac(A_M(j-1)+rhs3_Pt)-(w_MP(j-1)+w_MP3));
